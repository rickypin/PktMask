...
### 项目背景
**当前状态更新**: PktMask项目当前存在两套并行的处理架构：BaseProcessor系统（旧架构）和StageBase Pipeline系统（新架构）。载荷掩码功能已完成迁移，但IP匿名化和去重功能仍使用旧架构，导致架构复杂性和维护成本。

### 核心目标
- **完成架构统一**：将剩余的BaseProcessor组件迁移到StageBase架构
- **消除混合架构**：移除BaseProcessor系统和ProcessorRegistry桥接层
- **提升开发效率**：减少架构复杂性，简化维护工作
- **建立统一基础**：实现完全基于StageBase的统一架构

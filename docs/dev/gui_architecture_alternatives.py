...
class TkinterMainView(tk.Tk):  # 实际应该继承 IMainView
    ...
    def _create_options_section(self, parent, row) -> None:
        ...
        ttk.Checkbutton(options_frame, text="Remove Duplicates", 
                       variable=self.remove_dupes_var, command=self._on_options_changed).pack(side=tk.LEFT, padx=5)
        ttk.Checkbutton(options_frame, text="Anonymize IPs", 
                       variable=self.anonymize_ips_var, command=self._on_options_changed).pack(side=tk.LEFT, padx=5)
        ttk.Checkbutton(options_frame, text="Mask Payloads", 
                       variable=self.mask_payloads_var, command=self._on_options_changed).pack(side=tk.LEFT, padx=5)

...
#### 2.1 多层IP提取功能
**任务清单**:
- [ ] 增强`HierarchicalAnonymizationStrategy`的IP扫描能力
- [ ] 修改`_prescan_addresses`方法支持多层IP
- [ ] 实现IP层级标识 (外层/内层)
...
### 第三阶段：Payload裁切增强 (2-3周)

#### 3.1 内层TCP会话识别
**任务清单**:
- [ ] 增强`get_tcp_session_key`函数
- [ ] 实现封装内TCP会话检测
- [ ] 支持多层TCP会话管理
...
#### 测试组1：封装检测测试
**文件**: `tests/unit/test_encapsulation_detector.py`
...
#### 测试组2：协议栈解析测试
**文件**: `tests/unit/test_protocol_parser.py`
...
#### 测试组3：IP匿名化增强测试
**文件**: `tests/unit/test_enhanced_anonymization.py`
...
#### 测试组4：载荷裁切增强测试
**文件**: `tests/unit/test_enhanced_trimming.py`
...
周次  │ 阶段              │ 主要任务                    │ 里程碑
─────┼─────────────────┼──────────────────────────┼─────────────
1-3   │ 基础架构搭建       │ 核心组件开发、基础封装支持    │ 框架完成
4-6   │ IP匿名化增强       │ 多层IP提取、分层匿名化处理   │ IP处理完成
7-9   │ Payload裁切增强    │ 内层TCP识别、载荷裁切处理   │ 载荷处理完成
10-12 │ 集成测试与优化     │ 综合测试、性能优化、文档    │ 功能发布
```

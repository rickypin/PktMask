...
class TLSProtocolMarker(ProtocolMarker):
    ...
    def analyze_file(self, pcap_path: str, config: Dict[str, Any]) -> KeepRuleSet:
        ...
        try:
            ...
            self.logger.info(
                f"TLS analysis completed, took {analysis_time:.2f} seconds, "
    ...
    def _scan_tls_messages(self, pcap_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 执行第二阶段扫描
            completed_segments = run_hidden_subprocess(
                cmd_segments,
                check=True,
                text=True,
                capture_output=True,
    ...
    def _merge_tls_scan_results(
        ...
        self.logger.debug(
            ...
            _dbg_reassembled_added,
            _dbg_segments_added,
            _dbg_ct_present_count,
            _dbg_seg_only_count,
        )
    ...
    def _determine_direction_from_values(
        self, stream_id: str, src_ip: str, src_port: int, dst_ip: str, dst_port: int
    ) -> str:
        ...
        if (str(src_ip), int(src_port)) < (str(dst_ip), int(dst_port)):
            ...
            canonical_forward = {
                "src_ip": str(dst_ip),
                "dst_ip": str(src_ip),
                "src_port": int(dst_port),
                "dst_port": int(src_port),
            }
        # 缓存该流的canonical方向
    ...
    def _generate_keep_rules(
        self, tls_packets: List[Dict[str, Any]], tcp_flows: Dict[str, Dict[str, Any]]
    ) -> KeepRuleSet:
    ...
    def _generate_fallback_rules_from_packets(
        self,
            ...
            if self._is_tls_fragment(packet):
                ...
                if self._is_applicationdata_fragment(packet):
                    ...
                    self.logger.warning(
                        ...
                        direction,
                        ct,
                        bool(layers.get("tls.segment.data")),
                        (payload_hex[:10] if payload_hex else ""),
                    )
                    continue
                else:
                    # 其他类型片段：完全保留
                    ...
                    if rule:
                        ruleset.add_rule(rule)
                        self.logger.debug(
                            f"Generated fragment full preserve rule: Frame {frame_number} "
                            f"seq {rule.seq_start}-{rule.seq_end}"
                        )
                    continue
    ...
    def _create_keep_rule_with_range(
        self,
        stream_id: str,
        direction: str,
        seq_start: int,
    ...
    def _create_tls23_header_rule(
        self, stream_id: str, direction: str, tcp_seq: int, frame_number: Any
    ) -> Optional[KeepRule]:
    ...
    def _create_keep_rule_for_tls_message(
        self,
        stream_id: str,
        direction: str,
        ...
        try:
            ...
            if is_application_data and not preserve_full_application_data:
                ...
                self.logger.debug(
                    f"Created TLS-23 message #{message_index} header keep rule: "
    ...
    def _create_full_preserve_rule(
        self, packet: Dict[str, Any], tcp_flows: Dict[str, Dict[str, Any]]
    ) -> Optional[KeepRule]:
        """为TLS片段创建完全保留规则
    ...
    def _validate_cross_segment_record(self, content_type: int, declared_length: int, actual_length: int) -> bool:
        ...
        try:
            ...
            if declared_length > MAX_TLS_RECORD_LENGTH:
                self.logger.warning(
                    f"TLS record declared length too large: {declared_length} > {MAX_TLS_RECORD_LENGTH}"
                )
                return False
            ...
            if actual_length < declared_length * 0.1:  # 实际长度不应该小于声明长度的10%
                self.logger.warning(
                    f"Cross-segment record actual length too small: {actual_length} < {declared_length * 0.1}"
                )
                return False
        ...
        try:
            ...
            if is_cross_segment:
                ...
                if rule_length > MAX_CROSS_SEGMENT_RULE_LENGTH:
                    self.logger.warning(
                        f"Cross-segment rule length too large: {rule_length} > {MAX_CROSS_SEGMENT_RULE_LENGTH}"
                    )
                    return False

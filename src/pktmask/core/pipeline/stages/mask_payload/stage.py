...
class MaskPayloadStage(StageBase):
    ...
    支持两种处理模式：
    1. Processor Adapter Mode (默认): 使用 TSharkEnhancedMaskProcessor + ProcessorAdapter
    ...
    def __init__(self, config: Optional[Dict[str, Any]] | None = None):
        ...
        self._processor_adapter: Optional[Any] = None  # ProcessorAdapter，延迟导入
    ...
    def _initialize_processor_adapter_mode(self, config: Dict[str, Any]) -> None:
        """初始化处理器适配器模式（TSharkEnhancedMaskProcessor + ProcessorAdapter）"""
        try:
            # 创建增强处理器并用适配器包装
    ...
    def _process_with_processor_adapter_mode(self, input_path: Path, output_path: Path) -> StageStats:
        """使用 TSharkEnhancedMaskProcessor + ProcessorAdapter 进行处理"""
        ...
        try:
            ...
            # ProcessorAdapter.process_file 返回 StageStats，直接返回
            return result

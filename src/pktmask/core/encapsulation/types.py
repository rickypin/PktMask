...
class EncapsulationResult:
    ...
    def has_vlan(self) -> bool:
        """是否包含VLAN封装"""
        return self.encap_type in [EncapsulationType.VLAN, EncapsulationType.DOUBLE_VLAN] or \
               any(layer.encap_type in [EncapsulationType.VLAN, EncapsulationType.DOUBLE_VLAN] 
    ...
    def is_complex_encapsulation(self) -> bool:
        """是否为复杂封装 (多层或复合)"""
        return self.encap_type == EncapsulationType.COMPOSITE or \
               self.total_depth > 3 or \
               self.has_multiple_ips()

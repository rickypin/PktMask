...
class EncapsulationDetector:
    ...
    def _detect_plain(self, packet: Packet) -> bool:
        ...
        try:
            ...
            has_encap = (
                packet.haslayer(Dot1Q)
                or packet.haslayer(MPLS)
                or packet.haslayer(GRE)
                or (VXLAN and packet.haslayer(VXLAN))
            )
    ...
    def _resolve_encapsulation_type(
        ...
        for encap_type in priority_order:
            if encap_type in detected_types:
                if len(detected_types) > 1:
                    self.logger.info(
                        f"Detected composite encapsulation, selecting highest priority type: {encap_type.value}"
                    )
                return encap_type

...
ALL_KNOWN_SUFFIXES = ['-Deduped', '-Replaced']  # Centralized list of all possible suffixes

def select_files(subdir_path: str, current_suffix: str) -> Tuple[List[str], str]:
    """
    根据处理后缀选择要处理的文件。
    ...
    other_suffixes = [s for s in ALL_KNOWN_SUFFIXES if s != current_suffix]
    ...
    for suffix in reversed(ALL_KNOWN_SUFFIXES):
    ...
    # 原始文件是指不含任何已知后缀的文件
    raw_files = [f for f in all_files if not any(s in f for s in ALL_KNOWN_SUFFIXES)]
    if raw_files:

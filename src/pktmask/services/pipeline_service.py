...
def build_pipeline_config(
    ...
    options = service.create_options_from_gui(
        dedup_checked=enable_dedup,
        anon_checked=enable_anon,
        mask_checked=enable_mask
    )
...
def process_single_file(
    ...
    try:
        ...
        # 发送处理完成事件
        if progress_callback:
            progress_callback(PipelineEvents.FILE_END, {
                'path': input_file,
        ...
        return {
            ...
            'stage_stats': [stats.model_dump() if hasattr(stats, 'model_dump') else stats.__dict__ for stats in result.stage_stats],
        ...
        for i, input_file in enumerate(pcap_files):
            try:
                ...
                result = process_single_file(
                    executor, input_file, output_file,
                    progress_callback, verbose
                )

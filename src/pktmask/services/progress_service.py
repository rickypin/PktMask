...
class ProgressService:
    ...
    def start_file(self, filename: str):
        ...
        self._emit_event(PipelineEvents.FILE_START, {
            'path': filename,
        ...
        if self.style in [ProgressStyle.DETAILED, ProgressStyle.RICH]:
            ...
            self._print(f"📄 [{self.state.processed_files + 1}/{self.state.total_files}] "
    ...
    def update_stage(self, stage_name: str, stats: Dict[str, Any]):
        ...
        self._emit_event(PipelineEvents.STEP_SUMMARY, {
        ...
        if self.style == ProgressStyle.RICH:
            ...
            self._print(f"  ⚙️  [{stage_name}] {packets_processed:,} packets, "
    ...
    def complete_file(self, filename: str, success: bool = True):
        ...
        self._emit_event(PipelineEvents.FILE_END, {
            'path': filename,
            'success': success,
        ...
        if self.style in [ProgressStyle.SIMPLE, ProgressStyle.DETAILED, ProgressStyle.RICH]:
            if success:
    ...
    def complete_processing(self):
        ...
        if self.style != ProgressStyle.NONE:
            ...
            if self.state.processed_files == self.state.total_files:
                ...
                self._print(f"⚠️  Processed {self.state.processed_files}/{self.state.total_files} files "
                           f"({failed} failed)")
...
def create_cli_progress_callback(
    ...
    progress_service = create_progress_service(
        "detailed" if verbose else "simple"
    )

...
class PerformanceMetrics(BaseModel):
    ...
    @classmethod
    def calculate(cls, total_files: int, total_packets: int, total_bytes: int, 
                  duration_seconds: float, memory_mb: float = 0.0, cpu_percent: float = 0.0) -> 'PerformanceMetrics':
        """计算性能指标"""
...
class ReportData(BaseModel):
    ...
    @classmethod
    def from_statistics(cls, stats: StatisticsData, step_results: StepResultData,
                       title: str = "处理报告") -> 'ReportData':
        """从统计数据创建报告"""
        summary = ProcessingSummary(
            ...
            success_rate=(stats.metrics.files_processed / stats.metrics.total_files_to_process * 100.0) 
                        if stats.metrics.total_files_to_process > 0 else 0.0
        )

...
from pktmask.core.encapsulation.types import (
    EncapsulationResult, IPLayerInfo, PayloadInfo, 
    EncapsulationType, EncapsulationError
)
...
class ProcessingAdapter:
    ...
    def analyze_packet_for_payload_processing(self, packet: Packet) -> Dict[str, Any]:
        ...
        try:
            ...
            payload_analysis = {
                ...
                'is_encrypted': (encap_result.innermost_payload.is_encrypted 
                               if encap_result.innermost_payload else False),
                'transport_protocol': (encap_result.innermost_payload.protocol 
                                     if encap_result.innermost_payload else None),
                'payload_depth': (encap_result.innermost_payload.layer_depth 
                                if encap_result.innermost_payload else 0),
                'tcp_session_info': self._extract_tcp_session_info(encap_result),
                'processing_hints': self._generate_payload_processing_hints(encap_result),
                'original_packet': packet,
                'encap_result': encap_result
            }
    ...
    def extract_ips_for_anonymization(self, ip_analysis: Dict[str, Any]) -> List[Tuple[str, str, str]]:
        """
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'encapsulation_ratio': (self.stats['encapsulated_packets'] / max(1, self.stats['total_packets'])),
            'multi_ip_ratio': (self.stats['multi_ip_packets'] / max(1, self.stats['total_packets'])),
            'error_ratio': (self.stats['processing_errors'] / max(1, self.stats['total_packets']))
        }
    ...
    def _update_encap_stats(self, encap_result: EncapsulationResult):
        ...
        self.stats['encap_type_counts'][encap_type_name] = self.stats['encap_type_counts'].get(encap_type_name, 0) + 1
    
    def _generate_ip_processing_hints(self, encap_result: EncapsulationResult) -> Dict[str, Any]:
        """生成IP处理提示"""
        hints = {
    ...
    def _generate_payload_processing_hints(self, encap_result: EncapsulationResult) -> Dict[str, Any]:
        """生成载荷处理提示"""
        hints = {
            'requires_deep_inspection': encap_result.encap_type != EncapsulationType.PLAIN,
    ...
    def _extract_tcp_session_info(self, encap_result: EncapsulationResult) -> Optional[Dict[str, Any]]:
        """提取TCP会话信息"""
        if not encap_result.innermost_payload:
            return None
        ...
        return {
            ...
            'seq': getattr(payload.layer_object, 'seq', 0) if payload.layer_object else 0,

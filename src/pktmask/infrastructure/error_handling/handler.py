...
from .recovery import ErrorRecoveryManager, RecoveryResult, RecoveryAction, get_recovery_manager
...
class ErrorHandler:
    ...
    def _log_error(self, error: PktMaskError, context: ErrorContext) -> None:
        ...
        try:
            # 基本错误记录
            log_exception(error, logger_name=context.component or 'error_handler', 
    ...
    def _generate_error_report(self, error: PktMaskError, context: ErrorContext, 
    ...
    def _notify_user(self, error: PktMaskError, context: ErrorContext, 
                    recovery_result: Optional[RecoveryResult]) -> None:
        """通知用户错误信息"""
        # 这里可以根据错误严重性决定通知方式
        # 对于高严重性错误，可能需要显示对话框
    ...
    def _call_error_callbacks(self, error: PktMaskError, context: ErrorContext, 
                             recovery_result: Optional[RecoveryResult]) -> None:
        """调用错误回调函数"""

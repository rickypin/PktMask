...
class TestEnhancedIPAnonymization:
    ...
    def test_prescan_vlan_packets(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            packets = [
                ...
                Ether()
                / Dot1Q(vlan=200)
                / IP(src="172.16.2.1", dst="172.16.2.2")
                / TCP(),
                Ether()
                / Dot1Q(vlan=100)
                / IP(src="172.16.1.3", dst="172.16.1.4")
                / TCP(),
            ]
            wrpcap(test_file, packets)
            ...
            assert self.strategy._encap_stats["total_packets_scanned"] == 3
            assert (
                self.strategy._encap_stats["encapsulated_packets"] >= 3
            )  # 所有包都是封装包
    ...
    def test_prescan_mixed_packets(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                self.strategy._encap_stats["encapsulated_packets"] >= 1
            )  # 至少1个封装包
    ...
    def test_create_mapping_with_encapsulation_stats(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mapping = self.strategy.create_mapping(
                files_to_process, temp_dir, error_log
            )
    ...
    def test_anonymize_plain_packet(self):
        """测试匿名化普通数据包"""
        # 设置映射
        self.strategy._ip_map = {"192.168.1.1": "10.0.0.1", "192.168.1.2": "10.0.0.2"}
    ...
    def test_anonymize_vlan_packet(self):
        """测试匿名化VLAN封装数据包"""
        # 设置映射
        self.strategy._ip_map = {"172.16.1.1": "10.10.1.1", "172.16.1.2": "10.10.1.2"}
    ...
    def test_anonymize_packet_not_in_mapping(self):
        """测试匿名化映射中不存在的IP"""
        # 设置有限映射
        self.strategy._ip_map = {"192.168.1.1": "10.0.0.1"}
...
class TestEncapsulationStatistics:
    ...
    def test_stats_collection_during_prescan(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 验证统计总和
            total_categorized = stats["plain_packets"] + stats["encapsulated_packets"]
    ...
    def test_end_to_end_vlan_processing(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            packets = [
                ...
                Ether()
                / Dot1Q(vlan=100, prio=2)
                / IP(src="192.168.100.2", dst="192.168.100.1")
                / TCP(sport=8080, dport=80),
            ]
            wrpcap(test_file, packets)
            ...
            for i, packet in enumerate(packets):
                ...
                anonymized_packet, is_anonymized = self.strategy.anonymize_packet(
                    packet
                )
    ...
    def test_mixed_encapsulation_processing(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            packets = [
                ...
                Ether()
                / Dot1Q(vlan=200)
                / IP(src="172.20.1.1", dst="172.20.1.2")
                / TCP(),
                # 另一个普通包，相同网段
                Ether() / IP(src="10.1.1.3", dst="10.1.1.4") / TCP(),
            ]
            ...
            # 检查A.B段一致性
            assert (
                mapped_10_1_1_1.split(".")[0] == mapped_10_1_1_3.split(".")[0]
            )  # A段相同
            assert (
                mapped_10_1_1_1.split(".")[1] == mapped_10_1_1_3.split(".")[1]
            )  # B段相同

...
from src.pktmask.core.pipeline.stages.mask_payload_v2.masker.memory_optimizer import MemoryOptimizer
from src.pktmask.core.pipeline.stages.mask_payload_v2.masker.error_handler import ErrorRecoveryHandler, ErrorSeverity, ErrorCategory
from src.pktmask.core.pipeline.stages.mask_payload_v2.masker.data_validator import DataValidator
from src.pktmask.core.pipeline.stages.mask_payload_v2.masker.fallback_handler import FallbackHandler, FallbackMode
from src.pktmask.core.pipeline.stages.mask_payload_v2.marker.types import KeepRule, KeepRuleSet, FlowInfo
...
class TestPayloadMasker:
    ...
    def test_apply_keep_rules_partial_overlap(self):
        ...
        rule_data = {
            'bounds': [95, 105, 120, 130],
            'keep_set': {(95, 105), (120, 130)}
        }
    ...
    @patch('src.pktmask.core.pipeline.stages.mask_payload_v2.masker.payload_masker.PcapReader')
    @patch('src.pktmask.core.pipeline.stages.mask_payload_v2.masker.payload_masker.PcapWriter')
    def test_apply_masking_no_scapy(self, mock_writer, mock_reader):
        """测试没有scapy时的错误处理和降级处理"""
        # 模拟scapy不可用
        with patch('src.pktmask.core.pipeline.stages.mask_payload_v2.masker.payload_masker.PcapReader', None):
...
class TestIntegratedErrorHandling:
    ...
    def setup_method(self):
        ...
        self.config = {
            ...
            'data_validator': {
                'enable_checksum_validation': False  # 避免文件系统依赖
            },
            'fallback_handler': {
                'enable_fallback': True
            }

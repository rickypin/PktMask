...
class TestSequenceWrapAround:
    ...
    def test_logical_sequence_continuity(self):
        ...
        for i, (seq32, expected_seq64) in enumerate(zip(test_sequences, expected_logical)):
            ...
            if i > 0:
                assert expected_seq64 > expected_logical[i-1], f"逻辑序号必须递增: {expected_seq64} <= {expected_logical[i-1]}"
...
class TestCrossSegmentTLS:
    ...
    def test_tls_message_spanning_segments(self, scenario):
        ...
        if scenario == "handshake_across_2_segments":
            # TLS握手消息分割到2个TCP段
            segments = [
                {"seq_start": 1000, "seq_end": 1500, "tls_type": 22, "fragment": "first"},
                {"seq_start": 1500, "seq_end": 2000, "tls_type": 22, "fragment": "last"}
            ]
        elif scenario == "handshake_across_5_segments":
            # TLS握手消息分割到5个TCP段
            segments = [
                {"seq_start": 1000, "seq_end": 1200, "tls_type": 22, "fragment": "first"},
                {"seq_start": 1200, "seq_end": 1400, "tls_type": 22, "fragment": "middle"},
                {"seq_start": 1400, "seq_end": 1600, "tls_type": 22, "fragment": "middle"},
                {"seq_start": 1600, "seq_end": 1800, "tls_type": 22, "fragment": "middle"},
                {"seq_start": 1800, "seq_end": 2000, "tls_type": 22, "fragment": "last"}
            ]
        elif scenario == "application_data_fragmented":
            # 应用数据分片
            segments = [
                {"seq_start": 2000, "seq_end": 2300, "tls_type": 23, "fragment": "first"},
                {"seq_start": 2300, "seq_end": 2600, "tls_type": 23, "fragment": "middle"},
                {"seq_start": 2600, "seq_end": 2900, "tls_type": 23, "fragment": "last"}
            ]
        else:  # mixed_messages_fragmented
            # 混合消息类型分片
            segments = [
                {"seq_start": 3000, "seq_end": 3200, "tls_type": 22, "fragment": "complete"},
                {"seq_start": 3200, "seq_end": 3500, "tls_type": 23, "fragment": "first"},
                {"seq_start": 3500, "seq_end": 3800, "tls_type": 23, "fragment": "last"},
                {"seq_start": 3800, "seq_end": 4000, "tls_type": 21, "fragment": "complete"}
            ]
    ...
    def test_incomplete_tls_records(self):
        ...
        # 模拟截断的TLS记录
        incomplete_records = [
            {"type": 22, "start": 1000, "expected_end": 2000, "actual_end": 1800, "truncated": True},
            {"type": 23, "start": 2000, "expected_end": 3000, "actual_end": 3000, "truncated": False}
        ]
        ...
        for record in incomplete_records:
            if record["truncated"]:
                ...
                partial_rule = KeepRule(
                    ...
                    metadata={"truncated": True, "expected_length": record["expected_end"] - record["start"]}
                )
